...
EFJC = physics.single_chain.efjc.thermodynamics.EFJC
...
class LegendreAsymptoticAlternative(unittest.TestCase):
    """Class for Legendre transformation tests.
    ...
    def test_force(self):
        ...
        for _ in range(parameters.number_of_loops):
            ...
            force = nondimensional_force * \
                parameters.boltzmann_constant*temperature/link_length
            end_to_end_length = \
                model.isotensional.asymptotic.alternative.end_to_end_length(
                    np.array(force),
                    temperature
                )
            force_out = \
                model.isometric.asymptotic.alternative.legendre.force(
                    np.array(end_to_end_length),
                    temperature
                )
            residual_abs = \
    ...
    def test_nondimensional_force(self):
        ...
        for _ in range(parameters.number_of_loops):
            ...
            temperature = \
                ...
                parameters.temperature_scale*(0.5 - np.random.rand())
            nondimensional_end_to_end_length_per_link = \
                model.isotensional.asymptotic.alternative. \
                nondimensional_end_to_end_length_per_link(
                    np.array(nondimensional_force),
                    temperature
                )
            nondimensional_force_out = \
                model.isometric.asymptotic.alternative.legendre. \
                nondimensional_force(
                    np.array(nondimensional_end_to_end_length_per_link),
                    temperature
                )
