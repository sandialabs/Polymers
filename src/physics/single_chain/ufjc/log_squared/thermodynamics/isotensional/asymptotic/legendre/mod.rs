...
/// The structure of the log-squared-FJC model thermodynamics in the isotensional ensemble approximated using an asymptotic approach and a Legendre transformation.
...
{
    -(nondimensional_force.sinh()/nondimensional_force).ln() - (1.0 + nondimensional_force/nondimensional_force.tanh()/nondimensional_link_stiffness/0.4).ln() + 0.5*nondimensional_link_stiffness*(1.0/lambert_w(&(-nondimensional_force/nondimensional_link_stiffness)).exp()).ln().powi(2) - 0.5*(2.0*PI*link_length.powi(2)/nondimensional_link_stiffness).ln() - (8.0*PI.powi(2)*hinge_mass*link_length.powi(2)*BOLTZMANN_CONSTANT*temperature/PLANCK_CONSTANT.powi(2)).ln() + nondimensional_force/nondimensional_force.tanh() - 1.0 + nondimensional_force.powi(2)/nondimensional_link_stiffness*((nondimensional_force.tanh() - 1.0/nondimensional_force.tanh() + 1.0/nondimensional_force)/(0.4*nondimensional_force.tanh() + nondimensional_force/nondimensional_link_stiffness))
}
...
/// The implemented functionality of the log-squared-FJC model thermodynamics in the isotensional ensemble approximated using an asymptotic approach and a Legendre transformation.
impl LOGSQUAREDFJC
{
    /// Initializes and returns an instance of the log-squared-FJC model thermodynamics in the isotensional ensemble approximated using an asymptotic approach and a Legendre transformation.
