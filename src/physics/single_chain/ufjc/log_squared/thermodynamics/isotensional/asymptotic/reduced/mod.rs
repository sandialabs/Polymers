...
/// The structure of the log-squared-FJC model thermodynamics in the isotensional ensemble approximated using a reduced asymptotic approach.
...
{
    1.0/nondimensional_force.tanh() - 1.0/nondimensional_force + 1.0/lambert_w(&(-nondimensional_force/nondimensional_link_stiffness)).exp() - 1.0
}
...
pub fn nondimensional_gibbs_free_energy_per_link(link_length: &f64, hinge_mass: &f64, nondimensional_link_stiffness: &f64, nondimensional_force: &f64, temperature: &f64) -> f64
{
    ...
    -(nondimensional_force.sinh()/nondimensional_force).ln() + 0.5*nondimensional_link_stiffness*lambda.ln().powi(2) - nondimensional_force*(lambda - 1.0) - 0.5*(2.0*PI*link_length.powi(2)/nondimensional_link_stiffness).ln() - (8.0*PI.powi(2)*hinge_mass*link_length.powi(2)*BOLTZMANN_CONSTANT*temperature/PLANCK_CONSTANT.powi(2)).ln()
}
...
/// The implemented functionality of the log-squared-FJC model thermodynamics in the isotensional ensemble approximated using a reduced asymptotic approach.
impl LOGSQUAREDFJC
{
    /// Initializes and returns an instance of the log-squared-FJC model thermodynamics in the isotensional ensemble approximated using a reduced asymptotic approach.
