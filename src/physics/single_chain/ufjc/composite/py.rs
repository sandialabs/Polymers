...
{
    let composite = PyModule::new(py, "composite")?;
    super::thermodynamics::py::register_module(py, composite)?;
    parent_module.add_submodule(composite)?;
    composite.add_class::<CUFJC>()?;
    Ok(())
}

/// The composite uFJC (CuFJC) model.
#[pyclass]
pub struct CUFJC
{
    ...
    #[pyo3(get)]
    pub thermodynamics: super::thermodynamics::py::CUFJC
}
...
impl CUFJC
{
    ...
    pub fn init(number_of_links: u8, link_length: f64, hinge_mass: f64, number_of_bonds: u8, bond_stiffness: f64, bond_energy: f64, bond_scission_energy: f64, bond_attempt_frequency: f64) -> Self
    {
        CUFJC
        {
            ...
            thermodynamics: super::thermodynamics::py::CUFJC::init(number_of_links, link_length, hinge_mass, number_of_bonds, bond_stiffness, bond_energy, bond_scission_energy, bond_attempt_frequency)
        }
    }
}
