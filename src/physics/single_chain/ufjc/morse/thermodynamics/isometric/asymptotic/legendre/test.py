...
MORSEFJC = physics.single_chain.ufjc. \
    morse.thermodynamics.isometric.asymptotic.legendre.MORSEFJC


def isotensional_nondimensional_end_to_end_length_per_link(
    link_length, link_stiffness, link_energy, nondimensional_force, temperature
):
    """Function to get the isotensional
    nondimensional end-to-end length per link.

    """
    return physics.single_chain.ufjc.morse.thermodynamics. \
        isotensional.asymptotic. \
        MORSEFJC(8, link_length, 1.0, link_stiffness, link_energy). \
    ...
    def test_force(self):
        ...
        for _ in range(parameters.number_of_loops):
            ...
            nondimensional_force_max = \
                np.sqrt(link_stiffness*link_energy/8) / \
