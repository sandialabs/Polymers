...
fn monte_carlo_random_configuration()
{
    ...
    let configuration = random_configuration::<NUMBER_OF_LINKS>(&mut rng).0;
...
{
    let mut rng = rand::thread_rng();
    let gamma = random_nondimensional_end_to_end_length::<NUMBER_OF_LINKS>(&mut rng).0/(NUMBER_OF_LINKS as f64);
    assert!(gamma >= 0.0);
    assert!(gamma <= 1.0);
}
