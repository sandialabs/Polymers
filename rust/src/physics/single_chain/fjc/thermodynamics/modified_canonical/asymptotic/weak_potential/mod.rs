...
impl ModifiedCanonicalAsymptoticWeakPotential for FJC
{
    ...
    {
        let nondimensional_force = potential_stiffness*potential_distance*self.link_length/BOLTZMANN_CONSTANT/temperature;
        -self.number_of_links_f64*BOLTZMANN_CONSTANT*temperature*ln_sinhc(&nondimensional_force) + 0.5*potential_stiffness*self.contour_length.powf(2.0)*(langevin(&nondimensional_force).powf(2.0) + (nondimensional_force.powf(-2.0) - (nondimensional_force.sinh()).powf(-2.0))/self.number_of_links_f64) - self.number_of_links_f64*BOLTZMANN_CONSTANT*temperature*ln(&(8.0*PI.powf(2.0)*self.hinge_mass*self.link_length.powf(2.0)*BOLTZMANN_CONSTANT*temperature/PLANCK_CONSTANT.powf(2.0)))
    }
    ...
    {
        let nondimensional_force = nondimensional_potential_stiffness*nondimensional_potential_distance/self.number_of_links_f64;
        -self.number_of_links_f64*ln_sinhc(&nondimensional_force) + 0.5*nondimensional_potential_stiffness*(langevin(&nondimensional_force).powf(2.0) + (nondimensional_force.powf(-2.0) - (nondimensional_force.sinh()).powf(-2.0))/self.number_of_links_f64) - self.number_of_links_f64*ln(&(8.0*PI.powf(2.0)*self.hinge_mass*self.link_length.powf(2.0)*BOLTZMANN_CONSTANT*temperature/PLANCK_CONSTANT.powf(2.0)))
    }
